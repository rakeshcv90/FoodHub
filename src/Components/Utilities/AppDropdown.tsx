...
export  type AppDropdownProps<T> = {
    data: Array<T>;
  ...
  const RenderItem = <T extends object>({
    ...
    return (
      <TouchableOpacity
        ...
        <AppText type="normal" value={String(item[textDisplayKey])} />
      </TouchableOpacity>
    );
  };
  ...
  const AppDropdown = <T extends object>({
    ...
    textDisplayKey,
    showItemIcons,
    listContainerHeight = DIMENSIONS.SCREEN_HEIGHT * 0.15,
    ...
    const remainingBelowSpace = DIMENSIONS.SCREEN_HEIGHT - viewPositionY;
